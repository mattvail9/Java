...
#include <string.h>
#include <stdio.h>
#include <stdlib.h> //include sttement for for exit function

#define SERVER_FTP_PORT 1994	//listen port - control
#define CLIENT_FTP_PORT 1995	//listen port - data
...
int svcInitServer (int *s);	//svcInitServer prototype
...
int bufferSize = 100; //buffer size for read/write file opertions
/*
 * main
 *
	...
	)
{
	...
	int dcListenSocket;		//listen socket for data connection
	int bytesReceived;		//received bytes for data connection file transfer
	...
	printf("Initialize data connection\n");
	...
	do
	{
		...
		char tempCmd[1024];
		strcpy(tempCmd, userCmd);		//Stores userCmd in tempCmd to send to server

		char *ptr = strtok(userCmd, " ");
		if(ptr != NULL)
		{
		...
		ptr = strtok(NULL, " ");		//get next token from userCmd
		if( ptr != NULL)
		{
			strcpy(argument, ptr);		//copy to argument
		}
		else
		{
			argument[0] = '\0';
		...
		if(strcmp(cmd, "send") == 0)	// if send cmd
		{	// send local file to server
			if(argument == NULL || argument[0] == '\0') 	//Check for valid argument
			{
				printf("501 Invlid number of arguments.\n");
				continue;
			}

			FILE *fptr = fopen(argument, "r");  //open local file in read mode
			if(fptr != NULL)
			{
				/* send the userCmd to the server */
				status = sendMessage(ccSocket, tempCmd, strlen(tempCmd)+1);  
				if(status != OK)
				{
				...
				printf("Client ftp is waiting to accept data connection.\n");
				dcSocket = accept(dcListenSocket, NULL, NULL);	//accept data connection from server
				if(dcSocket < 0)
				{
					perror("cannot accept data connection:");
				}
				else
				{
					int totalBytes = 0; //accumulator for bytes sent
					while( ! feof(fptr) && status == OK)  // if not end of file read from file
					{
						char temp[bufferSize];
						int bytesRead = fread(temp, sizeof(char), bufferSize, fptr);  // read from local file
						...
						{
							status = sendMessage(dcSocket, temp, bytesRead); // send file data server
						}
					}
					printf("%d bytes sent.\n", totalBytes);
			...
			else
			{
				...
				if( fptr != NULL)
				{
					do
					{
						char temp[bufferSize];
						bytesReceived = -1;
						status = receiveMessage(dcSocket, temp, bufferSize, &bytesReceived);	// receive data from server
						int bytesWritten = fwrite(temp, sizeof(char), bytesReceived, fptr);	// read from local file
						totalBytes = bytesReceived + totalBytes;
						if(bytesWritten < 0)
						{
				...
				{
					printf("error: recv - could not create file\n");
				}
				printf("closing data connection.\n");
				close(dcSocket);  // close data connection
			}
		}
		else //for any other case, receive reply from server
		{
			/* send the userCmd to the server */
			status = sendMessage(ccSocket, tempCmd, strlen(tempCmd)+1);  
			if(status != OK)
	...
	close(dcListenSocket);	// close data listen socket
